...
export const getThreads = createAsyncThunk('threads/getThreads', async (arg, { dispatch }) => {
  try {
...
export const postThreads = createAsyncThunk('threads/postThreads', async ({ requestBody }) => {
  const response = await axios.post(`/threads`, requestBody);
  return response.data;
});

export const upvoteThread = createAsyncThunk('threads/upvoteThread', async ({ threadId }) => {
  const response = await axios.post(`/threads/${threadId}/up-vote`);
  return response.data;
});

export const downvoteThread = createAsyncThunk('threads/downvoteThread', async ({ threadId }) => {
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(upvoteThread.pending, (state, action) => {
        ...
        if (threadDetail) {
          const newThread = {
            ...
            downVotesBy: threadDetail.downVotesBy.filter((user) => user !== userId)
          };
          ...
          const newThreads = state.threads.map((thread) =>
            thread.id === threadId
              ? {
                  ...
                  downVotesBy: thread.downVotesBy.filter((user) => user !== userId)
                }
              : thread
          );
        ...
        if (threadDetail) {
          ...
          const newThreads = state.threads.map((thread) =>
            thread.id === threadId
              ? {
                  ...thread,
                  upVotesBy: isUserMeUpvote ? [...thread.upVotesBy, userId] : thread.upVotesBy,
                  downVotesBy: thread.downVotesBy.filter((user) => user !== userId)
                }
              : thread
          );
      ...
      .addCase(neutralvoteThread.pending, (state, action) => {
        state.error = null;
        const { threadId, userId, isUserMeUpvote, isUserMeDownvote } = action.meta.arg;
        const threadDetail = state.thread;
        if (threadDetail) {
          const newThread = {
            ...threadDetail,
      ...
      .addCase(postThreadDetailComment.fulfilled, (state, action) => {
        if (state.thread) {
          const newComment = [action.payload.data.comment, ...state.thread.comments];
          ...
          const newComments = threadDetail.comments.map((comment) => {
            if (comment.id === commentId) {
              return {
                ...
                downVotesBy: comment.downVotesBy.filter((user) => user !== userId)
              };
            }
            return comment;
          });
          ...
          const newComments = threadDetail.comments.map((comment) => {
            if (comment.id === commentId) {
              return {
                ...comment,
                downVotesBy: comment.downVotesBy.filter((user) => user !== userId),
                upVotesBy: isUserMeUpvote ? [...comment.upVotesBy, userId] : comment.upVotesBy
              };
            }
      ...
      .addCase(neutralvoteComment.pending, (state, action) => {
        const { commentId, userId, isUserMeUpvote, isUserMeDownvote } = action.meta.arg;
        const threadDetail = state.thread;
        if (threadDetail) {
          const newComments = threadDetail.comments.map((comment) => {
            if (comment.id === commentId) {
      ...
      .addCase(neutralvoteComment.rejected, (state, action) => {
        state.status = 'failed';
        const { userId, commentId, isUserMeUpvote, isUserMeDownvote } = action.meta.arg;
        const threadDetail = state.thread;
        if (threadDetail) {
          const newComments = threadDetail.comments.map((comment) => {
            if (comment.id === commentId) {
